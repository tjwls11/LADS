...
def check_ssl(base_url: str) -> list[dict]:
    ...
    if parsed.scheme == "http":
        ...
        findings.append(misconfig_finding(
            ...
            status=0,
            confidence=HIGH,
            evidence="site does not use HTTPS — traffic is transmitted in plaintext",
        ))
    ...
    try:
            ...
            with ctx_strict.wrap_socket(sock, server_hostname=hostname) as ssock:
                ...
                if protocol in _WEAK_PROTOCOLS:
                    ...
                    findings.append(misconfig_finding(
                        ...
                        status=0,
                        confidence=HIGH,
                        evidence=f"weak TLS version negotiated: {protocol} (TLS 1.2+ required)",
                    ))
                else:
                ...
                if not_after:
                    ...
                    if days_left < 0:
                        ...
                        findings.append(misconfig_finding(
                            ...
                            status=0,
                            confidence=HIGH,
                            evidence=f"SSL certificate expired {abs(days_left)} days ago (expired: {not_after})",
                        ))
                    elif days_left < 30:
                        ...
                        findings.append(misconfig_finding(
                            ...
                            status=0,
                            confidence=MEDIUM,
                            evidence=f"SSL certificate expires in {days_left} days ({not_after})",
                        ))
                    else:
        ...
        findings.append(misconfig_finding(
            ...
            status=0,
            confidence=HIGH,
            evidence=f"SSL certificate verification failed: {e.reason}",
        ))
        ...
        findings.append(misconfig_finding(
            ...
            status=0,
            confidence=MEDIUM,
            evidence=f"SSL handshake error: {e}",
        ))
